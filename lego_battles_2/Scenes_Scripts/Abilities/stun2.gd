...
func _ready():
	AUmanager.removefromabilitypool("stun2") #curren abil
	AUmanager.addtoabilitypool("stun3", 6) #lvl2
	AUmanager.add_to_inventory("stun2")
	AUmanager.remove_from_inventory("stun1")
	
	var removethis = get_node("../stun1")
	removethis.remove()
